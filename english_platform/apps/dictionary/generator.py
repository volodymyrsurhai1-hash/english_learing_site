...
def get_ai_client():
    """Отложенная инициализация клиента. Забирает ключ только в момент вызова функции."""
...
class Example(BaseModel):
    ...
    tag: Optional[str] = Field(default=None, description="Тег: прямое значение / переносное значение / вопрос / отрицание / сленг")


class WordMeaning(BaseModel):
    ru: str = Field(description="Русский перевод конкретного значения")
    context: Optional[str] = Field(default=None, description="Часть речи, область употребления или оттенок смысла")
...
class ConstructionSlot(BaseModel):
    ...
    description: str = Field(description="Описание, что именно подставляется (физические объекты, критика и т.д.)")
    examples: list[str] = Field(description="Список примеров подходящих слов: ['dust', 'crumbs', 'criticism']")


class ConstructionUse(BaseModel):
    ru: str = Field(description="Перевод этого применения или смысла")
    context: Optional[str] = Field(default=None, description="Контекст употребления (прямое действие, переносный смысл и т.д.)")
...
class SynonymGroup(BaseModel):
    level: str = Field(description="Уровень сложности синонимов (A1, A2, B1, B2, C1, C2)")
    synonyms: list[str] = Field(description="Список синонимов, относящихся к этому уровню")
...
class LinguisticAnalysis(BaseModel):
    ...
    cefr: Optional[str] = Field(default=None, description="Уровень сложности: A1, A2, B1, B2, C1, C2")
    ...
    meanings: list[WordMeaning] = Field(default_factory=list, description="ВСЕ значения слова с примерами. ОБЯЗАТЕЛЬНО ЗАПОЛНЯТЬ, если entry_type=='word'!")
    english_definition: Optional[str] = Field(default=None, description="Прямое определение слова на английском языке")
    ...
    uses: list[ConstructionUse] = Field(default_factory=list, description="ВСЕ применения и смыслы конструкции с примерами")
    variations: list[Variation] = Field(default_factory=list, description="Вариации порядка слов")
